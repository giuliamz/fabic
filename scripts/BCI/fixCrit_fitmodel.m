function [logLike,all] = fixCrit_fitmodel(parameters,parameterNames,dataVA,responseLoc)
% Simulates responses from the Fixed Criterion model for a given parameter
...
% Finally changed from BCI model to fixedCriterion model by David Meijer.
...
% Parameters
fixCrit = parameters(ismember(parameterNames,'fixCrit'));
...
if any(cellfun(@isempty,{fixCrit,xP,sigP,sigA,sigV,kernelWidth}))
    error('fixCrit_fitmodel:missingParameter','Missing parameter in fixCrit_fitmodel.')
end

%Throw an error if any of the strictly positive parameters is zero or smaller  
if any([fixCrit,sigP,sigA,sigV,kernelWidth] <= 0)
...
%     end
    ...
    % Apply fixed criterion --> David Meijer (16-5-2025)
    x_disparity = abs(xA-xV);
    sV_hat = (x_disparity <= fixCrit).*s_hat_common + (x_disparity > fixCrit).*sV_hat_indep;
    sA_hat = (x_disparity <= fixCrit).*s_hat_common + (x_disparity > fixCrit).*sA_hat_indep;
