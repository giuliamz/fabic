...
for iSubj = 1:nsubj
    ...
    load(fullfile(dataPath,'results',['s' subjID{iSubj} '_fixcrit_simulations_control_nosac_pool_best10']),'fm_bestlogLike');
    logLike_group(iSubj,3)=fm_bestlogLike{:};
    % model 4: fixCrit com action (1=com; 2=non-com)
    load(fullfile(dataPath,'results',['s' subjID{iSubj} '_fixcrit_simulations_control_nosac_sep_best10']),'fm_bestlogLike');
    logLike_group(iSubj,4)=fm_bestlogLike{:};
    % model 5: stoFus no com action
    load(fullfile(dataPath,'results',['s' subjID{iSubj} '_stofus_simulations_control_nosac_pool_best10']),'fm_bestlogLike');
    logLike_group(iSubj,5)=fm_bestlogLike{:};
    % model 6: stoFus com action (1=com; 2=non-com)
    load(fullfile(dataPath,'results',['s' subjID{iSubj} '_stofus_simulations_control_nosac_sep_best10']),'fm_bestlogLike');
