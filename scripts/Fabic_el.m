...
%P.quitkey=KbName('ESCAPE');
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    stimDeviceNr = audioDeviceIndices{strcmp('Speakers (Creative SB X-Fi)',audioDeviceNames)}; %cuffie lab
    %stimDeviceNr = audioDeviceIndices{strcmp('Speakers (High Definition Audio Device)',audioDeviceNames)}; %casse lab
...
%             end
            ...
            %Stop sound playback
            %PsychPortAudio('Stop', P.pahandle_stim); % waitForEndOfPlayback = 1 --> but it should have ended by now
...
%     Eyelink('StopRecording');
 ...
 Loglabel = {'communicative', 'response_cue', 'aud_loc', 'vis_loc', 'response', ...
 'RT', 'early'};
 Log = [Loglabel; Log];
 Filename = append('C:\Users\HPZ22018292\Documents\MATLAB\Fabic\data\Log_', S.subj_ID, '_', S.run, '.xlsx');
