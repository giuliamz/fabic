%% Compute WAV based on predictions of winning model (BCI Separated)
...
% conditions: 2 Action (Com, NCom) x 2 Resp (A, V)
nCond = 4;
...
for isub=1:nSubj
    ...
    if strcmp(exp,'exp1')
        file = (fullfile(dataPath,['s' subjID{isub} '_bci_simulations_avert_nosac_sep_best10']));
    elseif strcmp(exp,'exp2')
        file = (fullfile(dataPath,['s' subjID{isub} '_bci_simulations_control_nosac_sep_best10']));
    end
    load(file, 'bciSimulations');
    % Extract and reorganize necessary info
    for icond=1:9
        % repA Com
        pred_resp(icond,1,isub) = bciSimulations(ind_Com(icond)).sA_hat;
        % repA NCom
        pred_resp(icond,2,isub) = bciSimulations(ind_NCom(icond)).sA_hat;
        % repV Com
        pred_resp(icond,3,isub) = bciSimulations(ind_Com(icond)).sV_hat;
        % repV NCom
        pred_resp(icond,4,isub) = bciSimulations(ind_NCom(icond)).sV_hat;
    end
end
...
set(gca,'LineWidth',1);
saveas(gcf, fullfile(dataPath,['WAV_pred_' exp]), 'svg');